...
func (m *mockDB) getRows(table string, column string, value interface{}) ([]map[string]interface{}, error) {
	...
	if (table == "person" && column == "id" && value == 1) ||
		...
		result = append(result, map[string]interface{}{"id": 1, "legal_name": "Fred"})
	}
	if (table == "person" && column == "id" && value == 2) ||
		(table == "person" && column == "legal_name" && value == "Bob") {
		result = append(result, map[string]interface{}{"id": 2, "legal_name": "Bob"})
	}
	if (table == "person" && column == "id" && value == 3) ||
		(table == "person" && column == "legal_name" && value == "Alice") {
		result = append(result, map[string]interface{}{"id": 3, "legal_name": "Alice"})
	}
	if (table == "person" && column == "id" && value == 4) ||
		(table == "person" && column == "legal_name" && value == "Eve") {
		result = append(result, map[string]interface{}{"legal_name": "Eve", "id": 4})
	}

	if (table == "company" && column == "id" && value == 1) ||
		(table == "company" && column == "legal_name" && value == "Meta") {
		result = append(result, map[string]interface{}{"id": 1, "legal_name": "Meta", "parent_company_id": nil})
	}
	if (table == "company" && column == "id" && value == 2) ||
		(table == "company" && column == "legal_name" && value == "Alphabet") {
		result = append(result, map[string]interface{}{"id": 2, "legal_name": "Alphabet", "parent_company_id": nil})
	}
	if (table == "company" && column == "id" && value == 3) ||
		...
		result = append(result, map[string]interface{}{"id": 3, "legal_name": "Google", "parent_company_id": 2})
	}
	if (table == "company" && column == "id" && value == 4) ||
		...
		result = append(result, map[string]interface{}{"id": 4, "legal_name": "Facebook", "parent_company_id": 1})
	}
	if (table == "company" && column == "id" && value == 5) ||
		(table == "company" && column == "legal_name" && value == "YouTube") ||
		(table == "company" && column == "parent_company_id" && value == 2) {
		result = append(result, map[string]interface{}{"id": 5, "legal_name": "YouTube", "parent_company_id": 2})
	}
	...
	if table == "person_company" && column == "person_id" && value == 1 {
		result = append(result, map[string]interface{}{"person_id": 1, "company_id": 1, "permissions": `{"admin":true}`})
	}
	if table == "person_company" && column == "company_id" && value == 1 {
		result = append(result, map[string]interface{}{"person_id": 1, "company_id": 1, "permissions": `{"admin":true}`})
	}
	if table == "person_company" && column == "person_id" && value == 2 {
		result = append(result, map[string]interface{}{"person_id": 2, "company_id": 3, "permissions": `{"admin":false}`})
	}
	...
	if table == "purchase" && column == "person_id" && value == 2 {
		result = append(result, map[string]interface{}{"payment_token": `9cf973a1-63e1-4967-855e-87bdccf0a6f7`, "price_paid": 145.40203494, "person_id": 2, "company_id": 4})
	}
	if table == "purchase" && column == "company_id" && value == 4 {
		result = append(result, map[string]interface{}{"payment_token": `9cf973a1-63e1-4967-855e-87bdccf0a6f7`, "price_paid": 145.40203494, "person_id": 2, "company_id": 4})
	}
	if table == "purchase" && column == "person_id" && value == 3 {
		result = append(result, map[string]interface{}{"payment_token": `40c56909-6df9-45f9-adf9-d6b35093566f`, "price_paid": 57.3125, "person_id": 3, "company_id": 2})
	}
...
func (m *mockDB) insertRow(table_name string, columns []string, values []interface{}, auto_value string) (int, error) {
	index := -1
	if auto_value != "" {
		index = start_index
		start_index++
...
func TestDownload(t *testing.T) {
	...
	result, _ = download(&mockdb, download_options)
	expected_result = DatabaseDump{"company": {{"id": 5, "legal_name": "YouTube", "parent_company_id": 2}}}
...
func contains(s []map[string]interface{}, m map[string]interface{}) bool {
	for _, v := range s {
